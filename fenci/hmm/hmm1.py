...
def Prob_Array():
    for key in array_Pi:
        if array_Pi[key] == 0:
            ...
            array_Pi[key] = numpy.log(array_Pi[key] / line_num)
    for key0 in array_A:
        for key1 in array_A[key0]:
            if array_A[key0][key1] == 0.0:
                array_A[key0][key1] = -3.14e+100
            else:
                array_A[key0][key1] = numpy.log(array_A[key0][key1] / count_dic[key0])
    # print(array_A)
    for key in array_B:
        for word in array_B[key]:
            if array_B[key][word] == 0.0:
                ...
                array_B[key][word] = numpy.piplog(array_B[key][word] / count_dic[key])
